 ...
 * 图表数据转换选项
 */
export interface ChartDataConvertOptions {
  // 线条颜色相关
  lineColor?: string;
  color?: string;
  // 面积图颜色
...
export function convertChartData(
  ...
  options?: ChartDataConvertOptions
) {
  const PreData = [];
  const MiddleData = [];
 ...
 * OHLC数据转换为单值数据
 */
export function convertOhlcToSingleValue(
  data: OhlcData[],
